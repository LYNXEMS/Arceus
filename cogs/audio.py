...
class Audio:
    ...
    async def audioset_cachemax(self, size: int):
        """Ustala maksymalną pamięć podręczną"""
        if size < self._cache_min():
            await self.bot.say("Sorka, mamy minimalny limit {} MB.".format(self._cache_min()))
            return

        self.settings["MAX_CACHE"] = size
        await self.bot.say("Pamięć podręczna to teraz {} MB.".format(size))
        self.save_settings()
    ...
    async def audioset_emptydisconnect(self, ctx):
        ...
        if not noppl_disconnect:
            await self.bot.say("Jeśli kanał będzie pusty"
                               " bot się rozłączy po"
                               " 5 minutach.")
        else:
            await self.bot.say("Bot już nie będzie się automatycznie rozłaczał.")
        self.save_settings()

    @audioset.command(name="maxlength")
    @checks.is_owner()
    async def audioset_maxlength(self, length: int):
        """Ustala maksymalną długość piosenki"""
        if length <= 0:
            await self.bot.say("Wow, długość na minusie...chyba nie jesteś zbyt mądry.")
            return
        self.settings["MAX_LENGTH"] = length
        await self.bot.say("Ustalono na {} sekund.".format(length))
        self.save_settings()

    @audioset.command(name="player")
    @checks.is_owner()
    async def audioset_player(self):
        """Zmienia player muzyki"""
        self.settings["AVCONV"] = not self.settings["AVCONV"]
        if self.settings["AVCONV"]:
            await self.bot.say("Używasz FFMPEG.")
        else:
            await self.bot.say("Używasz AVCONV.")
        self.save_settings()
    ...
    async def audioset_status(self):
        ...
        if self.settings["TITLE_STATUS"]:
            await self.bot.say("Od teraz tytuły piosenek pokażą się jako gra bota.")
            # not updating on disable if we say disable
            #   means don't mess with it.
            await self._update_bot_status()
        else:
            await self.bot.say("Tytuły piosenek już nie będą pokazywane jako gra bota.")
        self.save_settings()
    ...
    async def audioset_volume(self, ctx, percent: int=None):
        ...
        if percent is None:
            ...
            msg = "Głośność to %d%%" % vol
        elif percent >= 0 and percent <= 200:
            self.set_server_setting(server, "VOLUME", percent)
            msg = "Głośnośc to teraz %d." % percent
            if percent > 100:
                msg += ("\nPoziomy głosu powyżej 100 mogą powodować błedy")
            ...
            msg = "Wybierz wartość między 0 a 100."
        await self.bot.say(msg)

    @audioset.command(pass_context=True, name="vote", no_pm=True)
    @checks.mod_or_permissions(manage_messages=True)
    async def audioset_vote(self, ctx, percent: int):
        """PRocenty potrzebne do przeskipowania piosenki."""
        server = ctx.message.server

        if percent < 0:
            await self.bot.say("Podaj wartość między 0 a 100.")
            return
        elif percent > 100:
            percent = 100
        ...
        if percent == 0:
            enabled = False
            await self.bot.say("Głosowanie wyłączone.")
        else:
            enabled = True
            await self.bot.say("PRocent potrzebny do przegłosowania skipa to {}%".format(percent))
    ...
    async def joinvoice(self, ctx):
        """Dołącza do kanału w którym jesteś"""
    ...
    async def pause(self, ctx):
        ...
        server = ctx.message.server
        if not self.voice_connected(server):
            await self.bot.say("Nie jestem podłączona.")
            return
    ...
    async def play(self, ctx, *, url_or_search_terms):
        ...
        try:
            ...
            await self.bot.say("Najpierw dołącz do kanału muzycznego.")
            return
        except UnauthorizedConnect:
            await self.bot.say("Nie mogę tam dołączyć.")
            return
        except UnauthorizedSpeak:
            await self.bot.say("Nie mogę mówić na twoim kanale.")
            return
        except ChannelUserLimit:
            await self.bot.say("Twój kanał jest pełny.")
            return
        ...
        if self.currently_downloading(server):
            await self.bot.say("Już coś pobieram!")
            return
        ...
        if self._match_any_url(url):
            if not self._valid_playable_url(url):
                await self.bot.say("Hello, this is not the link you're looking for.")
                return
        else:
            url = url.replace("/", "&#47")
            url = "[SEARCH:]" + url
    ...
    async def playlist_create(self, ctx, name):
        """Tworzy pusta playliste"""
        server = ctx.message.server
        author = ctx.message.author
        if not self._valid_playlist_name(name) or len(name) > 25:
            await self.bot.say("Ta nazwa playlisty jest niewłaściwa.")
            return
    ...
    async def playlist_add(self, ctx, name, url):
        ...
        if self._valid_playable_url(url):
            try:
                await self.bot.say("Przeliczanie piosenek.")
            ...
            self._save_playlist(server, name, playlist)
            await self.bot.say("Playlista '{}' zapisana. Utwory: {}".format(
                name, len(songlist)))
        else:
            await self.bot.say("Hello, this is not the link you're looking for.")
    ...
    async def playlist_append(self, ctx, name, url):
        """Przypisuje do playlisty."""
        author = ctx.message.author
        server = ctx.message.server
        if name not in self._list_playlists(server):
            await self.bot.say("Nie ma takiej playlisty.")
            return
        ...
        try:
            ...
            await self.bot.say("Nie jesteś autorem tej playlisty.")
        except InvalidURL:
            await self.bot.say("Hello, this is not the link you're looking for.")
        else:
            await self.bot.say("Zrobione.")
    ...
    async def playlist_list(self, ctx):
        ...
        if playlists:
            ...
            await self.bot.say("Nie ma playlist.")

    @playlist.command(pass_context=True, no_pm=True, name="queuenow")
    async def playlist_queuenow(self, ctx, url):
        """Dodaje do kolejki mimo grającej playlisty."""
        server = ctx.message.server
        if not self.voice_connected(server):
            await self.bot.say("Nie jestem podłączona do kanału.")
            return

        # We are connected somewhere
        if server.id not in self.queue:
            log.debug("Coś nie pykło.")
            raise VoiceNotConnected("Coś bardzo nie pykło.")

        # We have a queue to modify
        self._add_to_queue(server, url)

        await self.bot.say("Dodano do kolejki.")
    ...
    async def playlist_remove(self, ctx, name):
        ...
        if not self._valid_playlist_name(name):
            await self.bot.say("Coś jest nie tak z tą nazwą playlisty.")
            return

        if not self._playlist_exists(server, name):
            await self.bot.say("Hello, this is not the playlist you're looking for..")
            return
    ...
    async def playlist_start(self, ctx, name):
        ...
        if voice_channel is None:
            await self.bot.say("Dołącz wpierw do kanału głosowego.")
            return
        ...
        if self._playlist_exists(server, name):
            if not self.voice_connected(server):
                try:
                    ...
                    await self.bot.say("Nie mogę dołączyć do twego kanału.")
                    return
                except UnauthorizedSpeak:
            ...
            await self.bot.say("Dodano playlistę do kolejki.")
        else:
            await self.bot.say("Taka playlista nie istnieje.")

    @playlist.command(pass_context=True, no_pm=True, name="mix")
    async def playlist_start_mix(self, ctx, name):
        """Gra i mixuje playlistę."""
    ...
    async def _queue(self, ctx, *, url=None):
        """Dodaje piosenke do kolejki. Działa z grającymi już playlistami."""
        if url is None:
        ...
        if self._match_any_url(url):
            if not self._valid_playable_url(url):
                await self.bot.say("Hello, this is not the link you are looking for.")
                return
        else:
            url = "[SEARCH:]" + url
    ...
    async def _queue_list(self, ctx):
        ...
        server = ctx.message.server
        if server.id not in self.queue:
            await self.bot.say("Nic nie gra!")
            return
        ...
        await self.bot.say("Zbieram informacje...")
        ...
        if ctx.invoked_subcommand is None:
            if self.is_playing(server):
                if self.queue[server.id]["REPEAT"]:
                    msg = "Kolejka ma włączoną pętlę."
                else:
                    msg = "Pętla wyłączona."
                await self.bot.say(msg)
                await self.bot.say(
                    "Użyj `{}repeat toggle` aby to zmienić.".format(ctx.prefix))
            else:
                await self.bot.say("Nic nie gra.")

    @repeat.command(pass_context=True, no_pm=True, name="toggle")
    async def repeat_toggle(self, ctx):
        """Zmienia ustawienie pętli."""
        server = ctx.message.server
        ...
        if repeat:
            await self.bot.say("Włączono.")
        else:
            await self.bot.say("Wyłączono.")
            ...
            if msg.author.voice_channel == vchan:
                if self.can_instaskip(msg.author):
                    ...
                    if msg.author.id in self.skip_votes[server.id]:
                        ...
                        reply = "Usunąłem twój głos."
                    else:
                        self.skip_votes[server.id].append(msg.author.id)
                        reply = "Zagłosowałeś aby pominąć."
                ...
                await self.bot.say("Nie jesteś w kanale muzycznym.")
        else:
            await self.bot.say("Nie gram nic.")
    ...
    async def stop(self, ctx):
        """Zatrzymuje i usuwa kolejkę."""
        server = ctx.message.server
        if self.is_playing(server):
            if ctx.message.author.voice_channel == server.me.voice_channel:
                if self.can_instaskip(ctx.message.author):
                    await self.bot.say('BLUE FLAROWANIE WSZYSTKIEGO...')
                    self._stop(server)
                else:
                    await self.bot.say("HEJ! W KANALE TEŻ SĄ INNI!.")
            else:
                await self.bot.say("Nie jesteś na kanale muzycznym.")
        else:
            await self.bot.say("Nie gram nic.")
    ...
    async def yt_search(self, ctx, *, search_terms: str):
        ...
        await self.bot.say("*INTENSYWNE SZUKANIE BITU*")
