...
class Alias:
    ...
    async def alias(self, ctx):
        """Zarządzanie serwerowymi aliasami"""
        if ctx.invoked_subcommand is None:
            await send_cmd_help(ctx)

    @alias.command(name="dodaj", pass_context=True, no_pm=True)
    @checks.mod_or_permissions(manage_server=True)
    async def _add_alias(self, ctx, command, *, to_execute):
        """Dodaj alias komendzie

           Przykład: !alias add test flip @Twentysix"""
        server = ctx.message.server
        command = command.lower()
        if len(command.split(" ")) != 1:
            await self.bot.say("Aliasy nie mogą być wielowyrazowe")
            return
        if self.part_of_existing_command(command, server.id):
            await self.bot.say('Alias nie może nazywać się jak komenda')
            return
        ...
        if command not in self.bot.commands:
            ...
            await self.bot.say("Alias '{}' dodany.".format(command))
        else:
            await self.bot.say("Nie mogę dodać '{}' ponieważ to prawdziwa komenda".format(command))

    @alias.command(name="help", pass_context=True, no_pm=True)
    async def _help_alias(self, ctx, command):
        """Próbuje wywołać help dla danego aliasa"""
        server = ctx.message.server
        if server.id in self.aliases:
            ...
            if command in server_aliases:
                ...
                await self.bot.say("Ten alias nie istnieje.")

    @alias.command(name="pokaż", pass_context=True, no_pm=True)
    async def _show_alias(self, ctx, command):
        """Pokazuje jaką komende wywołuje alias."""
        server = ctx.message.server
        if server.id in self.aliases:
            server_aliases = self.aliases[server.id]
            if command in server_aliases:
    ...
    @alias.command(name="lista", pass_context=True, no_pm=True)
    async def _alias_list(self, ctx):
        """Podaje listę aliasów na serverze

        Odpowiada na priv"""
        server = ctx.message.server
        if server.id in self.aliases:
            message = "```Lista aliasów:\n"
            for alias in sorted(self.aliases[server.id]):
                if len(message) + len(alias) + 3 > 2000:
                    await self.bot.whisper(message)
                    message = "```\n"
                message += "\t{}\n".format(alias)
            if message != "```Lista aliasów:\n":
                message += "```"
                await self.bot.whisper(message)
            else:
                await self.bot.say("Ten server nie ma aliasów.")
