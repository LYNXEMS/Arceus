...
class Nsfw:
    ...
    async def nsfw(self, ctx):
        ...
        if discord.utils.get(server.roles, name="NSFW") in author.roles:
            await self.bot.remove_roles(author, discord.utils.get(server.roles, name="NSFW"))
            ...
            try:
                ...
                if answ == "Tak":
                    ...
                    await self.bot.say("Brak odpowiedzi lub niepoprawna odpowiedź, jeśli chcesz ponownie spróbować uzyskać dostęp, ponownie wpisz komendę.")
                    return
            except:
                await self.bot.say("Brak odpowiedzi lub niepoprawna odpowiedź, jeśli chcesz ponownie spróbować uzyskać dostęp, ponownie wpisz komendę.")
                return
    ...
    async def kolor(self, ctx, kolor):
        ...
        if discord.utils.get(server.roles, name="Znany") in author.roles:
            if kolor == "czerwony":
                ...
                await self.bot.remove_roles(author, discord.utils.get(server.roles, name="❤F"))
            ...
            if kolor == "fioletowy":
                ...
                await self.bot.remove_roles(author, discord.utils.get(server.roles, name="❤Ż"))
                await self.bot.remove_roles(author, discord.utils.get(server.roles, name="❤N"))
                await self.bot.add_roles(author, discord.utils.get(server.roles, name="❤F"))
                await self.bot.say("Kolor Zmieniony")              
            else:
                return
        else:
